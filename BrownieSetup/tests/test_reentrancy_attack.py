...
def test_attack():
    ...
    assert savings_account_contract.balanceOf(user) == Wei("50 ether")
    assert savings_account_contract.balance() == Wei("150 ether")
    ...
    print("*****")
    print("*****Before attack******")
    print(f"SavingsAccountV2 balance {Wei(savings_account_contract.balance())}")
    print(f"Attacker's balance {Wei(attacker.balance())}")
    # ATTACK!!!
    investor_contract.attack({"from": attacker, "value": Wei('10 ether')})
    print("*****After attack******")
    print(f"SavingsAccountV2 balance {Wei(savings_account_contract.balance())}")
    print(f"Attacker's balance {attacker.balance()}")
